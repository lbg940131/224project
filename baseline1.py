...
from keras.layers import Input, Dense, GRU, Dropout, LSTM, Activation
from keras.layers import Bidirectional, Embedding, SpatialDropout1D, concatenate, GlobalAveragePooling1D, GlobalMaxPool1D, GlobalMaxPooling1D
...
def get_lstm_model(maxlen, max_features, embed_size, embedding_matrix, lr=0.0, lr_d=0.0, units=0, dr=0.0):
...
# def get_lr_model(maxlen, max_features, embed_size, embedding_matrix, lr=0.0, lr_d=0.0, units=0, dr=0.0):
# 	inp = Input(shape=(maxlen,))
# 	x = Embedding(max_features, embed_size, weights=[embedding_matrix])(inp)
# 	x = Dense(6, activation="sigmoid")(x)
# 	model = Model(inputs=inp, outputs=x)
...
def get_grucnn_model(maxlen, max_features, embed_size, embedding_matrix, lr=0.0, lr_d=0.0, units=0, dr=0.0):
    inp = Input(shape=(maxlen,)) 
    x = Embedding(max_features, embed_size, weights=[embedding_matrix])(inp)
...
def get_lstmcnn_model(maxlen, max_features, embed_size, embedding_matrix, lr=0.0, lr_d=0.0, units=0, dr=0.0):
    inp = Input(shape=(maxlen,)) 
    x = Embedding(max_features, embed_size, weights=[embedding_matrix])(inp)
    # https://www.tensorflow.org/api_docs/python/tf/keras/layers/SpatialDropout1D
    x = SpatialDropout1D(dr)(x)
...
if __name__ == "__main__":    
	...
	# model = get_baseline_model(maxlen, max_features, embed_size, embedding_matrix, lr=1e-3, lr_d=0, units=128, dr=0.2)

	# model = get_model(maxlen, max_features, embed_size, embedding_matrix, lr=1e-3, lr_d=0, units=128, dr=0.2)
	model = get_grucnn_model(maxlen, max_features, embed_size, embedding_matrix, lr=1e-3, lr_d=0, units=128, dr=0.2)
	...
	history = model.fit(X_t, y, batch_size=32, epochs=2)
